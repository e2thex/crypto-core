...
$(document).ready( function (){
  ...
  $("textarea").each(function() {
    ...
    $(this).bind("encrypt", function() {
      ...
      password.val("");
      $(this).val(rtext);
      $(this).trigger("setup_decrypt");

    }).bind("decrypt", function() {
      var etext = $(this).val();
      etext = etext.replace("<div class='decryptable'>","");
      etext = etext.replace("</div>","");
      password = $("#password_for_" + $(this).attr("id"))
      var text = AESDecryptCtr(etext, password.val(), 256);
      $(this).trigger("setup_encrypt");
      //password.val("");
      $(this).val(text);
    ...
    }).bind("setup_encrypt", function() {
      var id = $(this).attr("id");
      $(this).after("<input type='button' class = 'start-encrypt-button' value ='e' field ='" + id + " '>");
      $(".start-encrypt-button").click(function(e) {
        var id = $(this).attr("field");
        $("#" + id).trigger("start_encrypt",e);
        $(this).remove();
      });
    }).bind("setup_decrypt", function() {
      $(this).click(function(e) {$(this).trigger("start_decrypt",e).unbind("click");});

    });
    if ($(this).val().search("<div class='decryptable'>") == -1) {
      $(this).trigger("setup_encrypt");
    }
    else {
      $(this).trigger("setup_decrypt");
...
function passwordRequest(id,isEncrypt,e) {
  left = e.pageX;
  top = e.pageY;
  if (isEncrypt) {
    c = "encrypt-button";
  ...
  r = "<form class='password-request-wrapper' style ='position:absolute;top:" + top + "px; left:" + left + "px'>";
  r = r + "<input value ='' id ='password_for_" + id + "'><input type='submit' class = '" + c + "' value ='" + v + "' field ='" + id + " '>";
  r = r + "</form>";
  return r;
}
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
